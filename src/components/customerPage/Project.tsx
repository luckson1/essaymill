...
const ProjectCustomer = () => {
  ...
  const { data: project, isLoading } = api.project.getOneUserProject.useQuery({
    id,
  });
  ...
  const uploadToS3 = async (project: ProjectCustomer, filesList: FileList, type:Values["type"]) => {
    if (!filesList) {
      return null;
    }
...
const ctx=api.useContext()
  ...
  return (
    ...
    >
      ...
      {view === "messages" && (
       <MessagesUser projectId={projectId} userId={userId ?? ""}/>
      )}
      {view === "customerFiles" && (
