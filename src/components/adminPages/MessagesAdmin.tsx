...
const MessagesAdmin = ({
  ...
  const { data: messages, isLoading } = api.message.getMessages.useQuery({
    projectId,
  });
  ...
  const { data } = api.project.getAllUnreadMessages.useQuery(undefined, {
    refetchInterval: 10000,
  });
  useEffect(() => {
    if (data?.unreadMsgs && data?.unreadMsgs?.length > 0) {
      data?.unreadMsgs.forEach((message) =>
        markRead({ projectId: projectId, creatorId: message.creator.id })
      );
    }
