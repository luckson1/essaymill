...
import MessagesAdmin from "~/components/adminPages/MessagesAdmin";
import { api } from "~/utils/api";
import Skeleton from "../loadingState/Skeleton";


const AdminMessenger = ({userId}: {userId:string}) => {

  const [projectId, setProjectId] = useState<string>();
  const [value, setValue] = useState<string>("");
  const{data}=api.project.getAllUnreadMessages.useQuery()
  const [isContent, setIsContent] = useState(false);


const {mutate:getOrderNumber, isLoading, isError}=api.project.getProjectId.useMutation({
    onSuccess(data) {
...
<button className="btn w-full" role={"submit"}>
      ...
      >
        ...
        {isError && <div className="text-red-500 text-center"> Sorry, There is No such Order</ div >}
          { projectId && <MessagesAdmin projectId={projectId} userId={userId ?? ""}/> }
        </div>
      </div>
    </div>
  );
